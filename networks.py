...
class RSSM(nn.Module):
    def __init__(
        ...
        self._embed = embed
        self._device = device
        self._cont_stoch_size = cont_stoch_size
        ...
        self.cont_feat_layers.append(nn.Linear(self._stoch * self._discrete + self._deter, self._cont_stoch_size, bias=False))
        # self.cont_feat_layers.append(norm(self._cont_stoch_size, eps=1e-03))
        
        # self.cont_feat_layers.append(nn.Tanh())
        self._cont_feat_layers = nn.Sequential(*self.cont_feat_layers)
        self._cont_feat_layers.apply(tools.weight_init)
    ...
    def obs_step_by_prior(self, prev_post, now_prior, prev_action, true_embed, is_first, sample=True):
        # if shared is True, prior and post both use same networks(inp_layers, _img_out_layers, _ims_stat_layer)
        # otherwise, post use different network(_obs_out_layers) with prior[deter] and embed as inputs
        prev_action *= (1.0 / torch.clip(torch.abs(prev_action), min=1.0)).detach()
        ...
        if self._shared:
            true_post = self.img_step(prev_post, prev_action, true_embed, sample)
        else:
            if self._temp_post:
                x = torch.cat([now_prior["deter"], true_embed], -1)
            else:
                x = true_embed
            # (batch_size, prior_deter + embed) -> (batch_size, hidden)
            x = self._obs_out_layers(x)
