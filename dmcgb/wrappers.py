...
class DMCWrapper(core.Env):
    def __init__(
        ...
        if domain_name == "cup":  # Only domain with multiple words.
        ...
        if from_pixels:
            ...
            for key, value in self._env.observation_spec().items():
                if len(value.shape) == 0:
                    ...
                    shape = value.shape
                obs_spaces[key] = spaces.Box(-np.inf, np.inf, shape, dtype=np.float32)
            obs_spaces["image"] = spaces.Box(0, 255, (self._height, self._width, 3), dtype=np.uint8)
            self._observation_space = spaces.Dict(obs_spaces)
        else:
            raise "haven't support this"
    ...
    def step(self, action):
        ...
        for _ in range(self._frame_skip):
            ...
            if done:
                break
        time_step.observation.update({"image":self._get_obs(time_step)})
        self.current_state = _flatten_obs(time_step.observation)
        extra['discount'] = time_step.discount
        return time_step, reward, done, extra
