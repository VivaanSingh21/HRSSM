...
def make_env(
	...
	is_sensor_cs = mode == 'sensor_cs'
	if is_distracting_cs:
		import env.distracting_control.suite as dc_suite
		loaded_paths = [os.path.join(dir_path, 'DAVIS/JPEGImages/480p') for dir_path in ds_resource_path]
		for path in loaded_paths:
			if os.path.exists(path):
				paths.append(path)
	if is_sensor_cs:
		import env.sensor_control.suite as sc_suite
	env = dmcgb.make(
...
class ColorWrapper(gym.Wrapper):
	...
	def _load_colors(self):
		...
		self._colors = torch.load(f'env/data/{self._mode}.pt')
	...
	def _get_dmc_wrapper(self):
		_env = self.env
		while not isinstance(_env, dmcgb.wrappers.DMCWrapper) and hasattr(_env, 'env'):
			_env = _env.env
		assert isinstance(_env, dmcgb.wrappers.DMCWrapper), 'environment is not dmc2gym-wrapped'
...
def do_green_screen(x, bg):
	...
	im = TF.to_pil_image(torch.ByteTensor(x.copy()))
	im = im.convert('RGBA')
	pix = im.load()
...
class VideoWrapper(gym.Wrapper):
	...
	def _get_video_paths(self):
		video_dir = os.path.join('env/data', self._mode)
	...
	def _greenscreen(self, time_step):
		...
		if 'video' in self._mode:
			...
			bg = self._interpolate_bg(bg, time_step.observation['image'].shape[:-1]) # scale bg to observation size
			time_step.observation.update({"image": do_green_screen(time_step.observation['image'].transpose(2, 0, 1), bg).transpose(1, 2, 0)}) # apply greenscreen
			return time_step
		return time_step
